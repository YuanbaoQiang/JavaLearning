 ...
 *      |---HashMap: 作为Map的主要实现类：线程不安全的，效率高。可以存储null的key或者value
 *          |---LinkedHashMap：保证在遍历map元素时，可以按照添加的顺序实现遍历
 *                              原因：在原有的HashMap底层结构基础上，添加了一对指针，指向前一个和后一个元素。
 *                              对于频繁的遍历操作，此类执行效率会高与HashMap
 *      |---TreeMap：保证添加的key-value进行排序，实现排序遍历。此时考虑key的自然排序和定制排序
 *                      底层使用红黑树
 ...
 *    如果此位置上的数据不为空，意味着 此位置上存在一个或多个数据（以链表的形式存在），比较当前的key1和已经存在的一个或者多个数据的哈希值：
 *          如果key1的哈希值与已经存在的数据哈希值都不相同，此时key1-value1添加成功 --- 情况2
 *          如果key1的哈希值和已经存在的某一个数据的哈希值（key2-value2）相同，继续比较：调用key1所在类的equals()方法，比较：
 ...
 *
 *      在不断地添加过程中，会涉及到扩容问题，默认地扩容方式：扩容为原来容量地2倍，并将原有地数据复制过来。
 *
 *      当超出临界值，且要存放的位置为非空
 *
 *      jdk8 相较于jdk7在底层实现方面地不同：
 *      1. new HashMap()：底层没有创建一个长度为16的数组
 *      2. jdk 8 底层的数组是：Node[]，而非Entry[]
 *      3. 首次调用put()方法时，底层创建长度为16的数组
 *      4. jdk7底层结构：数组 + 链表 jdk8中底层结构：数组 + 链表 + 红黑树
 *          当数组的某一个索引位置上的元素以链表形式存在的数据个数 > 8 且当前的数组的长度 > 64时，此时此索引位置上的数据改为红黑树存储
 *
 *
 *
